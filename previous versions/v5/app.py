...
from engine import (
    TERM_CHECKPOINTS, CURRENCY_OPTIONS, LUMP_SUM_MODES,
    gterm, caption_html, inject_glossary_css, stepper,
    add_months, build_schedule, summarize, yearly_stats, cumulative_upfront, add_month_axis,
    Scenario, save_session, load_session,
)
...
if "scenarios" not in st.session_state:
    st.session_state.scenarios = load_session(SESSION_FILE)
...
st.markdown(
    f"""
    <div style="background:rgba(28,131,225,0.10); border-radius:8px; padding:14px 16px;
                margin-bottom:16px; font-size:14.5px; line-height:1.55;">
    📘 <strong>{gterm("Principal")}</strong> is the amount you borrow (and still owe) — interest is charged
    on this outstanding balance each period. <strong>{gterm("Interest")}</strong> is the lender's charge for
    lending it to you. Every payment splits between the two: early on it's mostly interest, later it's mostly
    principal, which is why the 'Cumulative interest vs principal' chart below curves the way it does.
    Hover any <span style="border-bottom:1px dotted #808495;">dotted-underlined</span> word for a definition.
    </div>
    """,
...
# ---------------------------------------------------------------------------
# Settings, scenarios & session — a single collapsible block instead of a sidebar
# ---------------------------------------------------------------------------

with st.expander("⚙️ Settings, scenarios & session", expanded=False):
    ...
    with sc_col1:
        ...
        edit_choice = st.selectbox("Edit existing scenario", edit_names)
    with sc_col2:
        st.write("")
        st.write("")
...
# ---------------------------------------------------------------------------
# Main page: scenario builder (collapsible sections, several can be open at once)
# ---------------------------------------------------------------------------

st.subheader("✏️ Editing: " + (edit_choice if edit_choice != "<new scenario>" else "new scenario"))
...
with st.expander("💰 Loan basics", expanded=True):
    ...
    with c2:
        ...
        method = st.radio(
            ...
            options=["daily_actual", "monthly"],
            index=0 if base.method == "daily_actual" else 1,
            format_func=lambda m: "Daily actual/365 (bank-style, default)" if m == "daily_actual" else "Monthly (rate/12)",
            help="Daily actual/365: interest = balance × rate/365 × days in that month — what most UK lenders "
            "(trackers/SVR) actually do. Monthly: interest = balance × rate/12, the simpler textbook "
            "approximation. Daily-actual is usually closer to your real statement.",
        )
...
with st.expander("🏦 Deposit & upfront costs", expanded=False):
    deposit = stepper(
    ...
    caption_html(
        f'One-off costs like stamp duty, legal/conveyancing fees, surveys, or moving costs — real cash spent '
        f'that isn\'t part of the loan. Give each one a name, amount, and the date it was (or will be) paid.'
    )
    uc_df_default = (
...
with st.expander("📈 Rate changes", expanded=False):
...
with st.expander("💵 Lump sums", expanded=False):
...
for col, s in zip(cols, active):
    ...
    with col:
        ...
        with st.expander("Advanced view"):
            st.caption("Mean monthly figures by year")
            ys = yearly_stats(df)
            ...
            for k in TERM_CHECKPOINTS:
                ...
                if not match.empty and k <= s.term_years + 1:
                    rows.append({
                        "Year": f"Year {k}",
                        "Mean payment": f"{CUR}{match['MeanPayment'].iloc[0]:,.0f}",
                        "Mean interest": f"{CUR}{match['MeanInterest'].iloc[0]:,.0f}",
                        "Mean principal": f"{CUR}{match['MeanPrincipal'].iloc[0]:,.0f}",
                    })
            if rows:
            ...
            info_rows = [
                ...
                {"Field": "Interest method", "Value": "Daily actual/365" if s.method == "daily_actual" else "Monthly (rate/12)"},
...
caption_html(
    f'The outstanding loan {gterm("balance", "Balance")} (\'what you still owe\') month by month, i.e. the '
    f'{gterm("principal", "Principal")} remaining. It falls faster than a straight line because early payments '
    f'are mostly {gterm("interest", "Interest")} and later payments are mostly principal. The top axis shows '
    f'months elapsed since the start.'
)
...
fig_balance.update_layout(yaxis_title=f"Outstanding balance ({CUR})", xaxis_title="Date", height=440, margin=dict(t=70))
...
fig_payment.update_layout(yaxis_title=f"Scheduled monthly payment ({CUR})", xaxis_title="Date", height=380, margin=dict(t=70))
add_month_axis(fig_payment, ref_start, max_months_all, min_date_all, max_date_all)
st.plotly_chart(fig_payment, use_container_width=True)

# --- Total cash outlay ---
st.markdown("#### Total cash outlay over time")
caption_html(
...
fig_outlay.update_layout(yaxis_title=f"Cumulative cash outlay ({CUR})", xaxis_title="Date", height=420, margin=dict(t=70))
...
st.markdown("#### Interest paid per year")
caption_html(f'How much of each calendar year\'s payments went purely to {gterm("interest", "Interest")}, not principal.')
fig_yearly = go.Figure()
for s in active:
    y = yearly_stats(schedules[s.name])
    fig_yearly.add_trace(go.Bar(x=y["Year"], y=y["Interest"], name=s.name))
fig_yearly.update_layout(barmode="group", yaxis_title=f"Interest ({CUR})", xaxis=dict(title="Year", fixedrange=True), height=380)
st.plotly_chart(fig_yearly, use_container_width=True)

# --- Cumulative interest vs principal ---
st.markdown("#### Cumulative interest vs principal repaid")
caption_html(
    f'How your total payments split between {gterm("interest", "Interest")} (the lender\'s charge for '
    f'borrowing) and {gterm("principal", "Principal")} (the loan amount you\'re paying back).'
)
fig_cum = go.Figure()
for s in active:
    df = schedules[s.name].copy()
...
fig_cum.update_layout(yaxis_title=f"Cumulative {CUR}", xaxis_title="Date", height=420, margin=dict(t=70))
