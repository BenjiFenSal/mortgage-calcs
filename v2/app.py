...
from engine import (
    CURRENCY_OPTIONS, GLOSSARY, LUMP_SUM_MODES, inject_glossary_css, stepper,
    build_schedule, build_offset_schedule, summarize, add_months, add_month_axis,
)

SESSIONS_FILE = Path(__file__).parent / "sessions" / "local_sessions.json"
SESSIONS_FILE.parent.mkdir(exist_ok=True)

st.set_page_config(page_title="Mortgage Dashboard", layout="wide", initial_sidebar_state="expanded")

# ---------------------------------------------------------------------------
# Login gate — everything below this point runs only for an authenticated user.
# Sessions are namespaced by st.user.email so nobody can see or overwrite anyone else's data.
# ---------------------------------------------------------------------------

if not session_store.auth_configured():
    st.title("🏡 MortgageHub")
    st.error(
        "Login isn't set up yet. An admin needs to add a Google OAuth client under `[auth]` in "
        "`.streamlit/secrets.toml` — see Part 2a of SETUP.md."
    )
    st.stop()

if not st.user.is_logged_in:
    st.title("🏡 MortgageHub")
    st.write("Sign in to save and load your own mortgage scenarios.")
...
# --- Donezo-inspired theme: green brand anchor + a distinguishable categorical set ---
PRIMARY = "#12805c"
PRIMARY_DARK = "#0b5c40"
ACCENT = "#e0862a"
# Ordered for maximum adjacent contrast: hue alternates warm/cool so neighbouring
# scenarios in a chart never land on two shades of the same colour.
PALETTE = ["#12805c", "#e0862a", "#2f6fed", "#c0392b", "#7c3aed", "#0e9488", "#be185d", "#b45309"]
...
st.markdown(
    ...
    /* sidebar */
    section[data-testid="stSidebar"] {{ background: #ffffff; border-right: 1px solid #edf0ee; }}
    section[data-testid="stSidebar"] .block-container {{ padding-top: 1.6rem; }}
    .brand {{ font-size: 19px; font-weight: 800; color: #10241d; margin-bottom: 22px; display:flex; align-items:center; gap:8px; }}
    .brand-badge {{ width:30px; height:30px; border-radius:9px; background:{PRIMARY}; display:inline-flex;
                    align-items:center; justify-content:center; color:#fff; font-size:16px; }}
    .nav-label {{ font-size: 11px; letter-spacing: .09em; color: #9aa3a0; font-weight: 700; margin: 18px 2px 6px 2px; text-transform: uppercase; }}
    section[data-testid="stSidebar"] label p {{ font-size: 14px !important; color: #33403c; }}
    section[data-testid="stSidebar"] [data-testid="stCheckbox"] {{ padding: 3px 6px; border-radius: 9px; }}
    section[data-testid="stSidebar"] [data-testid="stCheckbox"]:hover {{ background: #eef6f2; }}

    /* cards */
    div[data-testid="stVerticalBlockBorderWrapper"] {{
        background: #ffffff; border-radius: 18px !important;
        box-shadow: 0 1px 2px rgba(16,36,29,0.04), 0 6px 16px rgba(16,36,29,0.05);
        border: 1px solid #eef1ef !important;
    }}
    div[data-testid="stExpander"] {{
        background: #ffffff; border-radius: 18px !important;
        box-shadow: 0 1px 2px rgba(16,36,29,0.04), 0 6px 16px rgba(16,36,29,0.05);
        border: 1px solid #eef1ef !important; overflow: hidden; margin-bottom: 18px;
    }}
    div[data-testid="stExpander"] summary {{ font-weight: 700; font-size: 16px; color: #10241d; padding: 6px 2px; }}

    /* buttons */
    .stButton button {{ border-radius: 10px; font-weight: 600; letter-spacing: .1px; }}
    .stButton button[kind="primary"] {{ background: {PRIMARY}; border-color: {PRIMARY}; box-shadow: 0 4px 10px rgba(18,128,92,0.28); }}
    .stButton button[kind="primary"]:hover {{ background: {PRIMARY_DARK}; border-color: {PRIMARY_DARK}; }}

    [data-testid="stMetricValue"] {{ color: #10241d; font-weight: 800; }}
    [data-testid="stMetricLabel"] {{ color: #7c8985; font-weight: 600; font-size: 12.5px; }}
    h1, h2, h3, h4 {{ font-weight: 700 !important; color: #10241d; }}

    .verdict-card {{
        background: linear-gradient(135deg, {PRIMARY} 0%, {PRIMARY_DARK} 100%);
        color: #fff; border-radius: 14px; padding: 16px 20px; margin: 8px 0;
        box-shadow: 0 6px 16px rgba(18,128,92,0.28);
    }}
    .module-title {{ display:flex; align-items:center; gap:8px; margin-bottom: 4px; }}

    /* stat tiles */
    .stat-grid {{ display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin: 18px 0 22px 0; }}
    .stat-tile {{ background: #fff; border-radius: 18px; padding: 18px 20px; box-shadow: 0 1px 2px rgba(16,36,29,0.04), 0 6px 16px rgba(16,36,29,0.05); }}
    .stat-tile-dark {{ background: linear-gradient(135deg, {PRIMARY} 0%, {PRIMARY_DARK} 100%); color: #fff; box-shadow: 0 8px 20px rgba(18,128,92,0.32); }}
    .stat-top {{ display: flex; justify-content: space-between; align-items: center; font-size: 13px; font-weight: 600; opacity: .82; margin-bottom: 14px; }}
    .stat-badge {{ width: 26px; height: 26px; border-radius: 50%; background: rgba(255,255,255,.22); display: inline-flex;
                   align-items: center; justify-content: center; font-size: 13px; flex: none; }}
    .stat-badge-light {{ background: #eaf5ef; color: {PRIMARY}; }}
    .stat-value {{ font-size: 30px; font-weight: 800; margin-bottom: 4px; line-height:1.1; }}
    .stat-sub {{ font-size: 12px; opacity: .68; font-weight: 500; }}

    .page-header {{ display:flex; justify-content:space-between; align-items:flex-end; margin-bottom: 4px; }}
    .page-title {{ font-size: 28px; font-weight: 800; color: #10241d; margin-bottom: 2px; }}
    .page-sub {{ color: #7c8985; font-size: 14px; }}
    </style>
    """,
...
def render_mortgage_results(calc, deposit_toggle_key):
    ...
    for sc in scenarios:
        ...
        m = st.columns(4 if include_upfront else 3)
        m[0].metric("Monthly payment", fmt(df.iloc[0]["Payment"]))
        m[1].metric("Total interest", fmt(summ["total_interest"]))
        m[2].metric("Payoff date", summ["payoff_date"].isoformat() if summ["payoff_date"] else "—")
        if include_upfront:
            total_cash = summ["total_paid"] + sc["deposit"] + sum(c[1] for c in sc["upfront_costs"])
            m[3].metric("Total cash invested", fmt(total_cash))
...
def module_offset():
    ...
    for sc in scenarios:
        ...
        st.markdown(f"**{sc['name']}**")
        m1, m2 = st.columns(2)
        m1.metric("Interest saved", fmt(base_summ["total_interest"] - off_summ["total_interest"]))
        m2.metric("Break-even savings rate", f"{breakeven:.2f}%")
...
def module_deposit():
    ...
    for idx, sc in enumerate(scenarios):
        ...
        if reached:
            ...
            m1, m2 = st.columns(2)
            m1.metric("Time to target", f"{years}y {rem}m")
...
def module_comparefixed():
    ...
    for idx, sc in enumerate(scenarios):
        ...
        st.markdown(f"**{sc['name']}**")
        m1, m2 = st.columns(2)
        m1.metric("Monthly", fmt(df.iloc[0]["Payment"]))
        m2.metric(f"True cost ({sc['fixed_years']:g}y)", fmt(cost))
...
def module_ditchfix():
    ...
    for idx, sc in enumerate(scenarios):
        ...
        m1.metric("Stay cost", fmt(stay_cost))
        m2.metric("Switch cost", fmt(switch_cost))
        m3.metric("Net saving", fmt(saving))
        bar_names += [f"{sc['name']}: Stay", f"{sc['name']}: Switch"]
        bar_vals += [stay_cost, switch_cost]
        bar_colors += ["#c8cdca", PALETTE[idx % len(PALETTE)]]
...
def module_borrow():
    ...
    for idx, sc in enumerate(scenarios):
        ...
        st.markdown(f"**{sc['name']}**")
        m1, m2 = st.columns(2)
        m1.metric("Max loan", fmt(max_loan))
        m2.metric("Max property price", fmt(max_price))
...
# Backed by Google Sheets when configured (see session_store.py), else a local JSON file.
# ---------------------------------------------------------------------------

def load_all_sessions() -> list[dict]:
    return session_store.load_all_sessions(SESSIONS_FILE, CURRENT_USER)


def write_all_sessions(sessions: list[dict]):
    session_store.write_all_sessions(sessions, SESSIONS_FILE, CURRENT_USER)


def render_session_bar():
    st.session_state.setdefault("sessions_expanded", False)
    with st.expander("💾 Sessions & settings", expanded=st.session_state.sessions_expanded):
        st.caption(f"Storage: {session_store.storage_label()}")
        ...
        if st.button("💾 Save session", type="primary", disabled=not name.strip()):
            ...
            st.toast(f"Saved session '{name.strip()}'.", icon="💾")
            st.rerun()
        ...
        if not sessions:
            ...
            for sess in sorted(sessions, key=lambda s: s["saved_at"], reverse=True):
                ...
                if r3.button("Load", key=f"loadbtn_{sess['name']}"):
                    ...
                    st.session_state.sessions_expanded = True
                    st.toast(f"Loaded '{sess['name']}'.", icon="📂")
                    st.rerun()
...
# ---------------------------------------------------------------------------
# Sidebar navigation (Donezo-style menu)
# ---------------------------------------------------------------------------

with st.sidebar:
    st.markdown('<div class="brand"><span class="brand-badge">🏡</span> MortgageHub</div>', unsafe_allow_html=True)
    uc1, uc2 = st.columns([3, 1])
    uc1.caption(f"👤 {CURRENT_USER}")
    uc2.button("Log out", key="logout_btn", on_click=st.logout)
    st.markdown('<div class="nav-label">Calculators</div>', unsafe_allow_html=True)
    new_visible = []
    for key, icon, title in CALC_META:
        checked = CALC_LABELS[key] in st.session_state.visible_calcs
        val = st.checkbox(f"{icon}  {title}", value=checked, key=f"nav_{key}")
        if val:
            new_visible.append(CALC_LABELS[key])
    st.session_state.visible_calcs = new_visible

    st.markdown('<div class="nav-label">General</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="nav-label">Sessions</div>', unsafe_allow_html=True)
    st.caption(session_store.storage_label())
    saved_sessions = sorted(load_all_sessions(), key=lambda s: s["saved_at"], reverse=True)
    if saved_sessions:
        session_names = [s["name"] for s in saved_sessions]
        pick = st.selectbox("Load previous session", session_names, key="sidebar_session_pick", label_visibility="collapsed")
        if st.button("📂 Load previous session", use_container_width=True):
            chosen = next(s for s in saved_sessions if s["name"] == pick)
            st.session_state.state = copy.deepcopy(chosen["state"])
            st.session_state.currency_choice = chosen.get("currency", st.session_state.currency_choice)
            st.session_state.visible_calcs = chosen.get("visible_calcs", st.session_state.visible_calcs)
            st.session_state.gen += 1
            st.success(f"Loaded '{chosen['name']}'.")
            st.rerun()
    else:
        st.caption("No saved sessions yet — save one below to see it here.")
...
st.markdown(
    ...
    <div class="page-header">
      <div>
        ...
        <div class="page-sub">Plan, compare, and model your mortgage with ease.</div>
      </div>
    </div>
    """,
    unsafe_allow_html=True,
...
n_active = len(st.session_state.visible_calcs)
total_scenarios = sum(len(st.session_state.state[LABEL_TO_KEY[l]]["scenarios"]) for l in st.session_state.visible_calcs)
n_sessions = len(load_all_sessions())

st.markdown(
    f"""
    <div class="stat-grid">
      <div class="stat-tile stat-tile-dark">
        <div class="stat-top"><span>Calculators Active</span><span class="stat-badge">↗</span></div>
        <div class="stat-value">{n_active}</div>
        <div class="stat-sub">of 8 available</div>
      </div>
      <div class="stat-tile">
        <div class="stat-top"><span>Total Scenarios</span><span class="stat-badge stat-badge-light">↗</span></div>
        <div class="stat-value">{total_scenarios}</div>
        <div class="stat-sub">Across active calculators</div>
      </div>
      <div class="stat-tile">
        <div class="stat-top"><span>Saved Sessions</span><span class="stat-badge stat-badge-light">↗</span></div>
        <div class="stat-value">{n_sessions}</div>
        <div class="stat-sub">Stored on disk</div>
      </div>
      <div class="stat-tile">
        <div class="stat-top"><span>Interest Model</span></div>
        <div class="stat-value" style="font-size:22px;">Daily 365</div>
        <div class="stat-sub">Bank-style accrual</div>
      </div>
    </div>
    """,
    unsafe_allow_html=True,
...
if not st.session_state.visible_calcs:
    st.info("Tick a calculator in the sidebar to get started.")

st.session_state.setdefault("collapsed_tools", {})

for label in st.session_state.visible_calcs:
    key = LABEL_TO_KEY[label]
    icon, title = CALC_META[[m[0] for m in CALC_META].index(key)][1:]
    is_collapsed = st.session_state.collapsed_tools.get(key, False)
    with st.container(border=True):
        hcol1, hcol2 = st.columns([8, 1])
        with hcol1:
            st.markdown(f'<div class="module-title"><h4>{icon} {title}</h4></div>', unsafe_allow_html=True)
        with hcol2:
            if st.button("▸" if is_collapsed else "▾", key=f"collapse_{key}", help="Collapse" if not is_collapsed else "Expand"):
                st.session_state.collapsed_tools[key] = not is_collapsed
                st.rerun()
        if not is_collapsed:
            MODULE_FUNCS[key]()
